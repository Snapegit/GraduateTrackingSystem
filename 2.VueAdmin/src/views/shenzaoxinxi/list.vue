	...
	<div>
		<div class="app-contain">
			<div class="list_search_view">
				<el-form :model="searchQuery" class="search_form" >
					<div class="search_view">
						...
						<div class="search_box">
							<el-input class="search_inp" v-model="searchQuery.shenzaoxuexiao" placeholder="深造学校"
								clearable>
							</el-input>
						</div>
					</div>
					<div class="search_view">
						...
						<div class="search_box">
							<el-select
								class="search_sel"
								clearable
								v-model="searchQuery.shenzaozhuangtai" 
								placeholder="深造状态"
								>
								<el-option v-for="item in shenzaozhuangtaiLists" :label="item" :value="item"></el-option>
							</el-select>
						</div>
					</div>
					<div class="search_view">
				...
				<br>
				<div class="btn_view">
					<el-button type="success" @click="addClick" v-if="btnAuth('shenzaoxinxi','新增')">新增</el-button>
					<el-button  v-if=" btnAuth('shenzaoxinxi','查看')" type="info"  :disabled="selRows.length==1?false:true" @click="infoClick(null)">详情</el-button>
					<el-button type="primary" :disabled="selRows.length==1?false:true" @click="editClick" v-if=" btnAuth('shenzaoxinxi','修改')">修改</el-button>
					<el-button type="danger" :disabled="selRows.length?false:true" @click="delClick(null)"  v-if="btnAuth('shenzaoxinxi','删除')">删除</el-button>
				</div>
			</div>
			<br>
			<el-table
				...
				<el-table-column label="审核" v-if="btnAuth('shenzaoxinxi','审核')" :resizable='true' :sortable='true' align="left" header-align="left">
					<template #default="scope">
				...
				<el-table-column label="操作" width="300" :resizable='true' :sortable='true' align="left" header-align="left">
					<template #default="scope">
						<el-button type="info" v-if=" btnAuth('shenzaoxinxi','查看')" @click="infoClick(scope.row.id)">详情</el-button>
					</template>
				</el-table-column>
			</el-table>
			<el-pagination 
...
<script setup>
	...
	const getList = () => {
		...
		params['sort'] = 'id'
		params['order'] = 'desc'
		if(searchQuery.value.shenzaoxuexiao&&searchQuery.value.shenzaoxuexiao!=''){
			params['shenzaoxuexiao'] = '%' + searchQuery.value.shenzaoxuexiao + '%'
		}
		if(searchQuery.value.shenzaozhuangtai&&searchQuery.value.shenzaozhuangtai!=''){
			params['shenzaozhuangtai'] = searchQuery.value.shenzaozhuangtai
		}
	...
	//搜索
	const shenzaozhuangtaiLists = ref([])
	const getshenzaozhuangtaiLists = () => {
		shenzaozhuangtaiLists.value = '未开始,深造中,已完成'.split(',')
	}
	getshenzaozhuangtaiLists()
	const searchClick = () => {
	...
	const approvalClick = (row) => {
		let params = {
			id:row.id,
			shenzaoxuexiao: row.shenzaoxuexiao,
			shenzaozhuanye: row.shenzaozhuanye,
			shenzaoshijian: row.shenzaoshijian,
			shenzaozhuangtai: row.shenzaozhuangtai,
