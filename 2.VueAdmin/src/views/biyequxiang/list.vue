	...
	<div>
		<div class="app-contain">
			<div class="list_search_view">
				<el-form :model="searchQuery" class="search_form" >
					<div class="search_view">
						...
						<div class="search_box">
							<el-input class="search_inp" v-model="searchQuery.gongzuodanwei" placeholder="工作单位"
								clearable>
							</el-input>
						</div>
					</div>
					<div class="search_view">
						...
						<div class="search_box">
							<el-select
								class="search_sel"
								clearable
								v-model="searchQuery.danweixingzhi" 
								placeholder="单位性质"
								>
								<el-option v-for="item in danweixingzhiLists" :label="item" :value="item"></el-option>
							</el-select>
						</div>
					</div>
					<div class="search_view">
				...
				<br>
				<div class="btn_view">
					<el-button type="success" @click="addClick" v-if="btnAuth('biyequxiang','新增')">新增</el-button>
					<el-button  v-if=" btnAuth('biyequxiang','查看')" type="info"  :disabled="selRows.length==1?false:true" @click="infoClick(null)">详情</el-button>
					<el-button type="primary" :disabled="selRows.length==1?false:true" @click="editClick" v-if=" btnAuth('biyequxiang','修改')">修改</el-button>
					<el-button type="danger" :disabled="selRows.length?false:true" @click="delClick(null)"  v-if="btnAuth('biyequxiang','删除')">删除</el-button>
					<el-button type="warning" @click="echartClick1" v-if="btnAuth('biyequxiang','单位性质统计')">单位性质统计</el-button>
				</div>
			</div>
			<br>
			<el-table
				...
				<el-table-column label="审核" v-if="btnAuth('biyequxiang','审核')" :resizable='true' :sortable='true' align="left" header-align="left">
					<template #default="scope">
				...
				<el-table-column label="操作" width="300" :resizable='true' :sortable='true' align="left" header-align="left">
					<template #default="scope">
						<el-button type="info" v-if=" btnAuth('biyequxiang','查看')" @click="infoClick(scope.row.id)">详情</el-button>
					</template>
				</el-table-column>
			</el-table>
			<el-pagination 
		...
		<!-- 统计图弹窗 -->
		<el-dialog v-model="echartVisible" :title="'单位性质统计'" width="70%">
			<div  id="danweixingzhiEchart1" style="width:100%;height:600px;"></div>
...
<script setup>
	...
	const getList = () => {
		...
		params['sort'] = 'id'
		params['order'] = 'desc'
		if(searchQuery.value.gongzuodanwei&&searchQuery.value.gongzuodanwei!=''){
			params['gongzuodanwei'] = '%' + searchQuery.value.gongzuodanwei + '%'
		}
		if(searchQuery.value.danweixingzhi&&searchQuery.value.danweixingzhi!=''){
			params['danweixingzhi'] = searchQuery.value.danweixingzhi
		}
	...
	const getdanweixingzhiLists = () => {
		danweixingzhiLists.value = '政府单位,事业单位,企业单位'.split(',')
	}
	getdanweixingzhiLists()
	const searchClick = () => {
	...
	const echartClick1 = ()=>{
		...
		nextTick(()=>{
			var danweixingzhiEchart1 = echarts.init(document.getElementById("danweixingzhiEchart1"),'macarons');
			context?.$http({
				url: `biyequxiang/group/danweixingzhi`,
				method: 'get'
			}).then(res=>{
				...
				// 使用刚指定的配置项和数据显示图表。
				danweixingzhiEchart1.setOption(option);
				  //根据窗口的大小变动图表
				window.onresize = function() {
				    danweixingzhiEchart1.resize();
				};
			})
		})
	}
	...
	const approvalClick = (row) => {
		let params = {
			...
			danweixingzhi: row.danweixingzhi,
			zhiwei: row.zhiwei,
			xinzi: row.xinzi,
			tianbaoshijian: row.tianbaoshijian,
